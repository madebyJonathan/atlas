...
export default function WhoWeAre() {
  return (
    <section
      id="who-we-are"
      className="w-full bg-white flex items-center px-16 py-20 gap-16"
    >
      {/* Text Column */}
        ...
        >
          {stats.map((stat) => (
            <div key={stat.label} className="flex flex-col gap-1">
              <span className="font-nineties text-[40px] leading-none text-atlas-dark">
                {stat.value}
              </span>
              <span className="font-mono text-[13px] tracking-[0.05em] text-atlas-muted">
                {stat.label}
              </span>
            </div>
          ))}
      ...
      {/* Image Column */}
      <div className="w-[520px] h-[560px] rounded overflow-hidden flex-shrink-0">
        <Image
